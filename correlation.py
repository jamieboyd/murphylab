...
def getFromDatabase(query):
    db2 = pymysql.connect(host="localhost", user="root", db="murphylab", password='Ingwerbonbon10')
    cur2 = db2.cursor()
    try:
        cur2.execute(query)
        rows = cur2.fetchall()
...
def generateQuery(table):
    if table == "sessiontrials":
        query = """SELECT p.`Mouse`,p.`david`,p.`Fixation`,p.`sumtrials`,a.`correctcounts`, round((a.`correctcounts`/p.`sumtrials`)*100,2) AS '% Performance' FROM
